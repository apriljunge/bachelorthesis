...
track_graph = MetaGraph(DiGraph(), label_type=Int, vertex_data_type=Point, edge_data_type=Dict);
...
begin
	...
	for label in labels(track_graph)
		CairoMakie.scatter!(ga, track_graph[label], markersize = 5, color = :black, rasterize = 5)
	end

	for (vertex1, vertex2) in edge_labels(track_graph)
		lines!(ga, [track_graph[vertex1], track_graph[vertex2]])
		bracket!([(track_graph[vertex1], track_graph[vertex2])], text="speed", color=:transparent, width=0)
	end

	for vertex in labels(track_graph)
		all_neighbor_labels = union(outneighbor_labels(track_graph, vertex), inneighbor_labels(track_graph, vertex))

		if length(all_neighbor_labels) == 2
			bracket!([(track_graph[all_neighbor_labels[1]], track_graph[all_neighbor_labels[2]])], text="radius", offset=10)
		end
	end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "68a165ed23eb23aba703c3083b3dcc858685b189"
