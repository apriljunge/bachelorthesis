...
# ╔═╡ 64b68fc1-3c01-4757-9919-c6cce5d6c929
...
begin
	overpass_response_file = "./$relationID.overpass.json"

	# TODO check how old the file is
	if !isfile(overpass_response_file)
		response = String(HTTP.post("http://overpass-api.de/api/interpreter",body=overpass_string).body)
...
md"## Parse data
Opens file and parses date to process"
...
# ╔═╡ 926ba706-1116-4962-b84a-8c16dbad459c
nodes

# ╔═╡ 771acde9-74e3-42fa-9ed9-a0b4921c589d
begin
	used_nodes = Set([])
	
	for way in overpass_ways
		for node in way["nodes"]
			push!(used_nodes, node)
		end
	end
end

# ╔═╡ 16c996bd-0d65-4662-8a28-1f8b01149402
used_nodes_map = Dict(zip(used_nodes, collect(1:length(used_nodes))))

# ╔═╡ e0ff40d2-1f03-47b1-a52e-1cbd638e9053
begin
	g = DiGraph(length(nodes));
	for way in overpass_ways
		node_before = first(way["nodes"])
		for node in way["nodes"][2:end]
			add_edge!(g, used_nodes_map[node_before], used_nodes_map[node]);
		end
	end
end

# ╔═╡ bf0d1fb9-965a-4189-853b-e366ebd62849
begin
	using Plots, GraphRecipes
	graphplot(g, curves=false)
end

# ╔═╡ fe7df113-2c5b-43ca-b436-d7fc91ff8c61
begin
	has_self_loops(g)
end

# ╔═╡ 256d5ce2-ed12-4a45-9738-c1a00b4a9312
degree_histogram(g)


# ╔═╡ 80a7fb9c-7020-4aea-b8ee-b31bdaafcc6d
# ╠═╡ disabled = true
#=╠═╡
begin
	g = DiGraph(length(nodes));
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "3473ef0cd4a87c192cecd06d02cff85d28c2a258"
